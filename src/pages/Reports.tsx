...
const Reports: React.FC = () => {
  ...
  const [machineRevenueData, setMachineRevenueData] = useState<MachineRevenueItem[]>([]);
  ...
  useEffect(() => {
    const fetchFinancialData = async () => {
      ...
      try {
        ...
        const profitLoss = await Promise.all(periods.map(async (period) => {
          // Fetch revenue data
          ...
          // Calculate totals
          const totalRevenue = revenueData ? revenueData.reduce((sum, entry) => sum + Number(entry.cash_in) - Number(entry.cash_out), 0) : 0;
          const totalExpenses = expenseData ? expenseData.reduce((sum, entry) => sum + Math.abs(Number(entry.amount)), 0) : 0;
        ...
        if (machines) {
          const machineRevData = await Promise.all(machines.map(async (machine) => {
            const { data: machineHistory, error: historyError } = await supabase
              .from('machine_history')
              .select('cash_in, cash_out, revenue')
              .eq('machine_id', machine.id)
              .gte('recorded_at', startOfDay(subDays(now, 30)).toISOString());
            
            if (historyError) throw historyError;
            
            const cashIn = machineHistory ? machineHistory.reduce((sum, entry) => sum + Number(entry.cash_in), 0) : 0;
            const cashOut = machineHistory ? machineHistory.reduce((sum, entry) => sum + Number(entry.cash_out), 0) : 0;
    ...
    <PageLayout title="Reports">
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <p className="text-muted-foreground">Generate and view detailed financial reports</p>
          </div>
        ...
        <Alert>
          ...
          <AlertDescription>
            Generate comprehensive reports on revenue, expenses, profit/loss, machine performance,
            and customer activity. Filter by custom date ranges or use preset periods.
          </AlertDescription>
        </Alert>
        ...
        <Tabs defaultValue="profit-loss" value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="profit-loss">
            <Card>
              ...
              <CardContent>
                {loading ? (
                  <div className="py-8 text-center text-muted-foreground">Loading financial data...</div>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
          ...
          <TabsContent value="machine-revenue">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Detailed breakdown of revenue by machine</CardDescription>
                <div className="highlight-bar"></div>
              </CardHeader>
              <CardContent>
                {loading ? (
                  <div className="py-8 text-center text-muted-foreground">Loading machine data...</div>
                ) : machineRevenueData.length === 0 ? (
                  <div className="py-8 text-center text-muted-foreground">No machine data available</div>
                ) : (
                  <Table>
                    <TableHeader>
                      <TableRow>
                    ...
                    <TableBody>
                      {machineRevenueData.map((machine) => (
                        <TableRow key={machine.machine_id}>
                          ...
                          <TableCell className="text-right">${machine.cashIn.toLocaleString()}</TableCell>
                          <TableCell className="text-right">${machine.cashOut.toLocaleString()}</TableCell>
                          <TableCell className="text-right font-semibold text-game-success">
                            ${Math.abs(machine.revenue).toLocaleString()}
                          </TableCell>
                          <TableCell className="text-right">
                            {machine.cashIn > 0 ? Math.round((Math.abs(machine.revenue) / machine.cashIn) * 100) : 0}%
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                    <TableRow className="bg-muted/30 font-semibold">
                      ...
                      <TableCell className="text-right">
                        ${machineRevenueData.reduce((sum, item) => sum + item.cashIn, 0).toLocaleString()}
                      </TableCell>
                      <TableCell className="text-right">
                        ${machineRevenueData.reduce((sum, item) => sum + item.cashOut, 0).toLocaleString()}
                      </TableCell>
                      <TableCell className="text-right text-game-success">
                        ${machineRevenueData.reduce((sum, item) => sum + Math.abs(item.revenue), 0).toLocaleString()}
                      </TableCell>
                      <TableCell className="text-right">
                        {(() => {
                          const totalCashIn = machineRevenueData.reduce((sum, item) => sum + item.cashIn, 0);
                          const totalRevenue = machineRevenueData.reduce((sum, item) => sum + Math.abs(item.revenue), 0);
                          return totalCashIn > 0 ? Math.round((totalRevenue / totalCashIn) * 100) : 0;
                        })()}%
                      </TableCell>
                    </TableRow>
                  </Table>
          ...
          <TabsContent value="more-reports">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Comprehensive analysis and custom reporting</CardDescription>
                <div className="highlight-bar"></div>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card>
                    ...
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-4">
                        Analyze which machines are being used the most and during what hours
                      </p>
                  ...
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-base">Customer Spending Analysis</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-4">
                        Track customer spending patterns and identify your top spenders
                      </p>
                      <Button variant="outline" className="w-full">Generate Report</Button>
                    </CardContent>
                  </Card>
                  ...
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-base">Seasonal Trend Analysis</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-4">
                  ...
                  <Card>
                    <CardHeader className="pb-2">
                      <CardTitle className="text-base">Marketing Campaign ROI</CardTitle>
                    </CardHeader>
                    <CardContent>
                      <p className="text-sm text-muted-foreground mb-4">
                        Measure the effectiveness and return on investment for marketing campaigns
                      </p>
                      <Button variant="outline" className="w-full">Generate Report</Button>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-primary/5 border-primary/20">
                    <CardContent className="p-4">
                      <div className="flex items-start space-x-4">
                        ...
                        <div>
                          <h3 className="text-lg font-semibold mb-1">Premium SaaS Plan - $399/month</h3>
                          <p className="text-sm text-muted-foreground mb-2">
                            This software is offered as a premium SaaS solution with the following benefits:
                          </p>
